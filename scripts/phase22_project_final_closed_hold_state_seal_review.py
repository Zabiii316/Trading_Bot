...
OUT = Path("data/processed/phase22_project_final_closed_hold_state_seal_review.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_closed_hold_state_seal_review_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
REVIEW_FILE = ARCHIVE_DIR / "project_final_closed_hold_state_seal_review.json"

SEAL_FILES = [
    Path("data/processed/phase22_project_final_closed_hold_state_seal.json"),
    Path("runtime/phase22_project_final_closed_hold_state_seal_state.json"),
    Path("data/processed/phase22_project_archive/project_final_closed_hold_state_seal.json"),
]
...
seals = [load(p) for p in SEAL_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

closed_hold_state_seal_created = (
    any_true(seals, "project_final_closed_hold_state_seal_created")
    or any("PHASE_22_PROJECT_FINAL_CLOSED_HOLD_STATE_SEAL_CREATED" in str(x.get("decision", "")) for x in seals)
    or any(x.get("phase22_status") == "project_final_closed_hold_state_seal_created" for x in seals)
)

project_status = first_value(seals, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(seals, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(seals, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(seals, "selected_phase21_next_action", "remain_on_hold")
archive_lock_type = first_value(seals, "archive_lock_type", "logical_record_only")
os_file_locking = first_value(seals, "os_file_locking_applied", False)

evidence_count = first_value(seals, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(seals, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(seals, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "closed_hold_state_seal_present": any(p.exists() for p in SEAL_FILES),
    "project_final_closed_hold_state_seal_created": closed_hold_state_seal_created,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_CLOSED_HOLD_STATE_SEAL_REVIEW_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_29_project_final_closed_hold_state_seal_review",
    ...
    "project_final_closed_hold_state_seal_review_passed": passed,
    ...
    "decision": decision,
    "next_phase": "Phase 22.30 — Project Final End State Record",
}
...
print(f"project_final_closed_hold_state_seal_review_passed={passed}")
...
print("phase22_status=project_final_closed_hold_state_seal_review_complete")
