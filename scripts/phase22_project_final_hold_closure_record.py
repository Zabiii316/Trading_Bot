...
OUT = Path("data/processed/phase22_project_final_hold_closure_record.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_hold_closure_record_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
CLOSURE_FILE = ARCHIVE_DIR / "project_final_hold_closure_record.json"

SUMMARY_REVIEW_FILES = [
    Path("data/processed/phase22_project_final_seal_summary_review.json"),
    Path("runtime/phase22_project_final_seal_summary_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_seal_summary_review.json"),
]
...
reviews = [load(p) for p in SUMMARY_REVIEW_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

seal_summary_review_passed = (
    any_true(reviews, "project_final_seal_summary_review_passed")
    or any("PHASE_22_PROJECT_FINAL_SEAL_SUMMARY_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in reviews)
    or any(x.get("phase22_status") == "project_final_seal_summary_review_complete" for x in reviews)
)
...
checks = {
    ...
    "seal_summary_review_present": any(p.exists() for p in SUMMARY_REVIEW_FILES),
    "project_final_seal_summary_review_passed": seal_summary_review_passed,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_HOLD_CLOSURE_RECORD_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_26_project_final_hold_closure_record",
    ...
    "project_final_hold_closure_record_created": passed,
    "final_hold_closure_state": {
        "bot_execution_state": "fully_on_hold",
        "trading_approval_state": "not_approved",
        "paper_shadow_state": "not_started",
        "live_trading_state": "not_approved",
        "micro_live_state": "not_approved",
        "exchange_order_submission": False,
        "real_capital_usage": False,
        "archive_status": "logically_locked_and_closed",
        "next_action": "remain_on_hold"
    },
    ...
    "decision": decision,
    "next_phase": "Phase 22.27 — Project Final Hold Closure Review",
}

write(OUT, record)
write(RUNTIME_OUT, record)
write(CLOSURE_FILE, record)

print(f"Report written to: {OUT}")
print(f"Runtime closure written to: {RUNTIME_OUT}")
print(f"Closure file written to: {CLOSURE_FILE}")
print(f"safe_mode_active={safe_mode}")
print(f"project_final_hold_closure_record_created={passed}")
...
print("phase22_status=project_final_hold_closure_record_created")
