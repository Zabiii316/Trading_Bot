...
TEST_FILE = Path("data/processed/phase23_reopening/order_construction_safety_test.json")
...
adapter_records = [load(p) for p in ADAPTER_FILES]
...
micro_trade_notional = parse_positive(os.getenv("MICRO_TRADE_NOTIONAL_USDT"))
max_order_notional = parse_positive(os.getenv("MAX_ORDER_NOTIONAL_USDT"))
max_daily_loss = parse_positive(os.getenv("MAX_DAILY_LOSS_USDT"))
max_position_size = parse_positive(os.getenv("MAX_POSITION_SIZE_USDT"))
...
forbidden_keys = {"apikey", "secret", "signature", "timestamp", "recvwindow"}
payload_keys = {str(k).lower() for k in simulated_order_payload.keys()}
payload_contains_forbidden_keys = any(k in payload_keys for k in forbidden_keys)
...
if not micro_notional_within_order_limit:
    risk_findings.append("Micro notional is not within max order limit")
if not order_limit_within_position_limit:
    risk_findings.append("Max order limit is not within max position limit")
if payload_contains_forbidden_keys:
    risk_findings.append("Simulated payload contains forbidden signed/secret keys")
...
checks = {
    ...
    "phase23_10_adapter_validation_present": any(p.exists() for p in ADAPTER_FILES),
...
record = {
    ...
    "order_construction_safety_test_passed": passed,
    "test_checks": checks,
    "blockers": blockers,
...
print(f"order_construction_safety_test_passed={passed}")
