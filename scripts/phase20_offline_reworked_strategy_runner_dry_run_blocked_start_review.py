...
OUT = Path("data/processed/phase20_offline_reworked_strategy_runner_dry_run_blocked_start_review.json")
RUNTIME_OUT = Path("runtime/phase20_offline_reworked_strategy_runner_dry_run_blocked_start_review_state.json")
...
START_GATE_RUNTIME = Path("runtime/phase20_offline_reworked_strategy_runner_dry_run_start_gate_state.json")
START_GATE_FILE = Path("data/processed/phase20_strategy_rework/offline_reworked_strategy_runner_dry_run_start_gate.json")
...
start_runtime = load_json(START_GATE_RUNTIME)
start_file = load_json(START_GATE_FILE)
...
dry_run_start_gate_ready = (
    start_gate.get("dry_run_start_gate_ready") is True
    or start_runtime.get("dry_run_start_gate_ready") is True
    or start_file.get("dry_run_start_gate_ready") is True
)
...
selected_option = (
    ...
    or start_runtime.get("selected_option")
    or start_file.get("selected_option")
    or manual.get("selected_option")
)

selected_phase19_path = (
    start_gate.get("selected_phase19_path")
    or start_runtime.get("selected_phase19_path")
    or start_file.get("selected_phase19_path")
    or manual.get("selected_phase19_path")
)

selected_next_action = (
    start_gate.get("selected_next_action")
    or start_runtime.get("selected_next_action")
    or start_file.get("selected_next_action")
...
review_checks = {
    ...
    "approved_for_execution_false": start_gate.get("approved_for_execution") is False,
    "approved_for_paper_shadow_false": start_gate.get("approved_for_paper_shadow") is False,
    "approved_for_live_false": start_gate.get("approved_for_live") is False,
...
if blocked_start_review_passed:
    decision = "PHASE_20_OFFLINE_REWORKED_STRATEGY_RUNNER_DRY_RUN_BLOCKED_START_REVIEW_COMPLETE_MANUAL_APPROVAL_REQUIRED"
    next_phase = "Phase 20.14 — Offline Reworked Strategy Runner Dry Run Hold State Consolidation"
else:
    decision = "PHASE_20_OFFLINE_REWORKED_STRATEGY_RUNNER_DRY_RUN_BLOCKED_START_REVIEW_FAILED_REVIEW_REQUIRED"
    next_phase = "Phase 20.14 — Offline Runner Dry Run Blocked Start Fix"

review_record = {
    "phase": "phase_20_13_offline_reworked_strategy_runner_dry_run_blocked_start_review_record",
...
review_file = PHASE20_DIR / "offline_reworked_strategy_runner_dry_run_blocked_start_review.json"

write_json(review_file, review_record)
write_json(RUNTIME_OUT, review_record)

report = {
    "phase": "phase_20_13_offline_reworked_strategy_runner_dry_run_blocked_start_review",
    "generated_at_unix": int(time.time()),
    "scope": "blocked_start_review_only_no_dry_run_no_backtest_no_execution",
    "git_head": current_git_head,
    "safety_flags": flags,
    "safe_mode_active": safe_mode,
    ...
    "next_phase": next_phase,
    "safety_notes": [
        "This phase reviews the blocked dry-run start state only.",
