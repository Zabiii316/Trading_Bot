...
echo ""
echo "Review this manually. If safe, remove it with:"
echo "rm runtime/KILL_SWITCH_ACTIVE"
exit 1
fi
...
echo "Use Binance Futures TESTNET keys only."
exit 1
fi
...
echo "Set real Binance Futures TESTNET keys."
exit 1
fi

if [[ "${BINANCE_API_KEY}" == "your_testnet_api_key" || "${BINANCE_API_SECRET}" == "your_testnet_api_secret" ]]; then
echo "❌ Placeholder keys detected."
echo "Set real Binance Futures TESTNET keys."
exit 1
fi
...
--dry-run > runtime/phase16_testnet_pre_dryrun.txt
...
python scripts/run_binance_live_execution.py
--signal data/processed/live_signal_dryrun.json
--risk data/processed/live_risk_dryrun.json > runtime/phase16_testnet_execution_raw.txt 2>&1
RESULT_CODE=$?
set -e
...
submitted = '"submitted": true' in raw.lower()
blocked = "live trading disabled" in raw.lower()
api_error = "api error" in raw.lower() or "binance api error" in raw.lower()
