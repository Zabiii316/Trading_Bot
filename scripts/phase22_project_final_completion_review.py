...
OUT = Path("data/processed/phase22_project_final_completion_review.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_completion_review_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
REVIEW_FILE = ARCHIVE_DIR / "project_final_completion_review.json"

CLOSEOUT_FILES = [
    Path("data/processed/phase22_project_final_completion_closeout.json"),
    Path("runtime/phase22_project_final_completion_closeout_state.json"),
    Path("data/processed/phase22_project_archive/project_final_completion_closeout.json"),
]
...
closeouts = [load(p) for p in CLOSEOUT_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

completion_closeout_passed = (
    any_true(closeouts, "project_final_completion_closeout_passed")
    or any("PHASE_22_PROJECT_FINAL_COMPLETION_CLOSEOUT_COMPLETE" in str(x.get("decision", "")) for x in closeouts)
    or any(x.get("phase22_status") == "project_final_completion_closeout_complete" for x in closeouts)
)

project_status = first_value(closeouts, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(closeouts, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(closeouts, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(closeouts, "selected_phase21_next_action", "remain_on_hold")
archive_lock_type = first_value(closeouts, "archive_lock_type", "logical_record_only")
os_file_locking = first_value(closeouts, "os_file_locking_applied", False)

evidence_count = first_value(closeouts, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(closeouts, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(closeouts, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "completion_closeout_present": any(p.exists() for p in CLOSEOUT_FILES),
    "project_final_completion_closeout_passed": completion_closeout_passed,
...
decision = (
    "PHASE_22_PROJECT_FINAL_COMPLETION_REVIEW_COMPLETE_REMAIN_ON_HOLD_NOT_APPROVED_FOR_EXECUTION"
    if passed else
    "PHASE_22_PROJECT_FINAL_COMPLETION_REVIEW_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_23_project_final_completion_review",
    ...
    "decision": decision,
    "next_phase": "Phase 22.24 — Project Final Seal Summary",
}
...
print(f"project_final_completion_review_passed={passed}")
...
print("phase22_status=project_final_completion_review_complete")
