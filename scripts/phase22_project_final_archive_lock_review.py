...
REVIEW_FILE = ARCHIVE_DIR / "project_final_archive_lock_review.json"

LOCK_FILES = [
    Path("data/processed/phase22_project_final_archive_lock_record.json"),
    Path("runtime/phase22_project_final_archive_lock_record_state.json"),
    Path("data/processed/phase22_project_archive/project_final_archive_lock_record.json"),
]
...
locks = [load(p) for p in LOCK_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

lock_created = (
    any_true(locks, "project_final_archive_lock_record_created")
    or any("PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_RECORD_CREATED" in str(x.get("decision", "")) for x in locks)
    or any(x.get("phase22_status") == "project_final_archive_lock_record_created" for x in locks)
)

project_status = first_value(locks, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(locks, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(locks, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(locks, "selected_phase21_next_action", "remain_on_hold")
archive_lock_type = first_value(locks, "archive_lock_type", "logical_record_only")
os_file_locking = first_value(locks, "os_file_locking_applied", False)

evidence_count = first_value(locks, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(locks, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(locks, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "archive_lock_record_present": any(p.exists() for p in LOCK_FILES),
    "archive_lock_record_created": lock_created,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_REVIEW_FAILED_REVIEW_REQUIRED"
)
...
record = {
    ...
    "archive_lock_review_passed": passed,
...
print(f"archive_lock_review_passed={passed}")
