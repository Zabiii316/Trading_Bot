...
OUT = Path("data/processed/phase22_project_final_archive_lock_review.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_archive_lock_review_state.json")
PHASE22_DIR = Path("data/processed/phase22_project_archive")

ARCHIVE_LOCK = Path("data/processed/phase22_project_final_archive_lock_record.json")
ARCHIVE_LOCK_RUNTIME = Path("runtime/phase22_project_final_archive_lock_record_state.json")
ARCHIVE_LOCK_FILE = Path("data/processed/phase22_project_archive/project_final_archive_lock_record.json")
...
lock = load_json(ARCHIVE_LOCK)
lock_runtime = load_json(ARCHIVE_LOCK_RUNTIME)
lock_file_json = load_json(ARCHIVE_LOCK_FILE)
...
archive_lock_record_created = (
    lock.get("project_final_archive_lock_record_created") is True
    or lock_runtime.get("project_final_archive_lock_record_created") is True
    or lock_file_json.get("project_final_archive_lock_record_created") is True
)
...
project_status = lock.get("project_status") or lock_file_json.get("project_status")
...
if os_file_locking_applied is None:
    os_file_locking_applied = lock_file_json.get("os_file_locking_applied")

evidence_count = lock.get("evidence_file_count") or manifest.get("evidence_file_count", 0)
runtime_count = lock.get("runtime_file_count") or manifest.get("runtime_file_count", 0)
doc_count = lock.get("documentation_file_count") or manifest.get("documentation_file_count", 0)
...
review_checks = {
    ...
    "archive_lock_present": ARCHIVE_LOCK.exists(),
    "archive_lock_runtime_present": ARCHIVE_LOCK_RUNTIME.exists(),
    "archive_lock_file_present": ARCHIVE_LOCK_FILE.exists(),
    ...
    "phase22_previous_status_archive_lock_record_created": phase22_previous_status == "project_final_archive_lock_record_created",
    ...
    "monitoring_not_started": lock.get("monitoring_started") is False,
    "run_dry_run_now_false": lock.get("run_dry_run_now") is False,
    "run_backtest_now_false": lock.get("run_backtest_now") is False,
    "execution_allowed_false": lock.get("execution_allowed") is False,
    "approved_for_execution_false": lock.get("approved_for_execution") is False,
    "approved_for_paper_shadow_false": lock.get("approved_for_paper_shadow") is False,
    "approved_for_live_false": lock.get("approved_for_live") is False,
    "paper_shadow_not_started": lock.get("paper_shadow_started") is False,
    "paper_shadow_start_not_approved": lock.get("approved_for_paper_shadow_start") is False,
    "exchange_order_submission_disabled": lock.get("exchange_order_submission") is False,
    "micro_live_not_approved": lock.get("approved_for_micro_live_execution") is False,
    "real_live_not_approved": lock.get("approved_for_real_live_trading") is False,
}

blockers = [key for key, value in review_checks.items() if value is not True]
archive_lock_review_passed = all(review_checks.values())

if archive_lock_review_passed:
    decision = "PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_REVIEW_COMPLETE_REMAIN_ON_HOLD_NOT_APPROVED_FOR_EXECUTION"
    next_phase = "Phase 22.22 — Project Final Completion Closeout"
else:
    decision = "PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_REVIEW_FAILED_REVIEW_REQUIRED"
    next_phase = "Phase 22.22 — Project Final Archive Lock Review Fix"

review_file = PHASE22_DIR / "project_final_archive_lock_review.json"

record = {
    "phase": "phase_22_21_project_final_archive_lock_review_record",
...
report = {
    "phase": "phase_22_21_project_final_archive_lock_review",
    "generated_at_unix": int(time.time()),
    "scope": "archive_lock_review_only_no_os_file_lock_no_monitoring_no_dry_run_no_backtest_no_execution",
    "git_head": head,
    "git_branch": branch,
    "git_remote_origin": remote,
    "safety_flags": flags,
...
print(f"archive_lock_review_passed={archive_lock_review_passed}")
...
print("phase22_status=project_final_archive_lock_review_complete")
