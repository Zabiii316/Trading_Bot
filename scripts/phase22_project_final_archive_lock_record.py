...
LOCK_FILE = ARCHIVE_DIR / "project_final_archive_lock_record.json"

UPSTREAM_FILES = [
    Path("data/processed/phase22_project_final_archive_completion_review.json"),
    Path("runtime/phase22_project_final_archive_completion_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_archive_completion_review.json"),
]
...
upstreams = [load(p) for p in UPSTREAM_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

upstream_complete = (
    any_true(upstreams, "archive_completion_review_passed")
    or any("PHASE_22_PROJECT_FINAL_ARCHIVE_COMPLETION_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in upstreams)
    or any(x.get("phase22_status") == "project_final_archive_completion_review_complete" for x in upstreams)
)

project_status = first_value(upstreams, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(upstreams, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(upstreams, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(upstreams, "selected_phase21_next_action", "remain_on_hold")

evidence_count = first_value(upstreams, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(upstreams, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(upstreams, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "upstream_archive_completion_review_present": any(p.exists() for p in UPSTREAM_FILES),
    "upstream_archive_completion_review_complete": upstream_complete,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_RECORD_FAILED_REVIEW_REQUIRED"
)
...
record = {
    ...
    "project_final_archive_lock_record_created": passed,
