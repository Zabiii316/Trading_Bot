...
CLOSEOUT_FILE = ARCHIVE_DIR / "project_final_completion_closeout.json"

LOCK_REVIEW_FILES = [
    Path("data/processed/phase22_project_final_archive_lock_review.json"),
    Path("runtime/phase22_project_final_archive_lock_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_archive_lock_review.json"),
]
...
lock_reviews = [load(p) for p in LOCK_REVIEW_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

archive_lock_review_passed = (
    any_true(lock_reviews, "archive_lock_review_passed")
    or any("PHASE_22_PROJECT_FINAL_ARCHIVE_LOCK_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in lock_reviews)
    or any(x.get("phase22_status") == "project_final_archive_lock_review_complete" for x in lock_reviews)
)

project_status = first_value(lock_reviews, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(lock_reviews, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(lock_reviews, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(lock_reviews, "selected_phase21_next_action", "remain_on_hold")
archive_lock_type = first_value(lock_reviews, "archive_lock_type", "logical_record_only")
os_file_locking = first_value(lock_reviews, "os_file_locking_applied", False)

evidence_count = first_value(lock_reviews, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(lock_reviews, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(lock_reviews, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "archive_lock_review_present": any(p.exists() for p in LOCK_REVIEW_FILES),
...
decision = (
    "PHASE_22_PROJECT_FINAL_COMPLETION_CLOSEOUT_COMPLETE_REMAIN_ON_HOLD_NOT_APPROVED_FOR_EXECUTION"
    if passed else
    "PHASE_22_PROJECT_FINAL_COMPLETION_CLOSEOUT_FAILED_REVIEW_REQUIRED"
)
...
record = {
    ...
    "project_final_completion_closeout_passed": passed,
