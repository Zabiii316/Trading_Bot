...
OUT = Path("data/processed/phase22_project_final_completion_closeout.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_completion_closeout_state.json")
PHASE22_DIR = Path("data/processed/phase22_project_archive")

ARCHIVE_LOCK_REVIEW = Path("data/processed/phase22_project_final_archive_lock_review.json")
ARCHIVE_LOCK_REVIEW_RUNTIME = Path("runtime/phase22_project_final_archive_lock_review_state.json")
ARCHIVE_LOCK_REVIEW_FILE = Path("data/processed/phase22_project_archive/project_final_archive_lock_review.json")

ARCHIVE_LOCK_RECORD = Path("data/processed/phase22_project_final_archive_lock_record.json")
...
lock_review = load_json(ARCHIVE_LOCK_REVIEW)
lock_review_runtime = load_json(ARCHIVE_LOCK_REVIEW_RUNTIME)
lock_review_file = load_json(ARCHIVE_LOCK_REVIEW_FILE)
lock_record = load_json(ARCHIVE_LOCK_RECORD)
completion_review = load_json(ARCHIVE_COMPLETION_REVIEW)
...
archive_lock_review_passed = (
    lock_review.get("archive_lock_review_passed") is True
    or lock_review_runtime.get("archive_lock_review_passed") is True
    or lock_review_file.get("archive_lock_review_passed") is True
)

archive_lock_record_created = lock_record.get("project_final_archive_lock_record_created") is True
archive_completion_review_passed = completion_review.get("archive_completion_review_passed") is True
...
project_status = lock_review.get("project_status") or lock_review_file.get("project_status")
phase20_status = lock_review.get("phase20_status") or lock_review_file.get("phase20_status")
phase21_status = lock_review.get("phase21_status") or lock_review_file.get("phase21_status")
phase22_previous_status = lock_review.get("phase22_status") or lock_review_file.get("phase22_status")
selected_phase21_next_action = lock_review.get("selected_phase21_next_action") or lock_review_file.get("selected_phase21_next_action")

archive_lock_type = lock_review.get("archive_lock_type") or lock_review_file.get("archive_lock_type")
os_file_locking_applied = lock_review.get("os_file_locking_applied")
if os_file_locking_applied is None:
    os_file_locking_applied = lock_review_file.get("os_file_locking_applied")

evidence_count = lock_review.get("evidence_file_count") or manifest.get("evidence_file_count", 0)
runtime_count = lock_review.get("runtime_file_count") or manifest.get("runtime_file_count", 0)
doc_count = lock_review.get("documentation_file_count") or manifest.get("documentation_file_count", 0)
...
completion_checks = {
    ...
    "archive_lock_review_present": ARCHIVE_LOCK_REVIEW.exists(),
    "archive_lock_review_runtime_present": ARCHIVE_LOCK_REVIEW_RUNTIME.exists(),
    "archive_lock_review_file_present": ARCHIVE_LOCK_REVIEW_FILE.exists(),
    "archive_lock_record_present": ARCHIVE_LOCK_RECORD.exists(),
    ...
    "archive_lock_review_passed": archive_lock_review_passed,
    "archive_lock_record_created": archive_lock_record_created,
    ...
    "os_file_locking_not_applied": os_file_locking_applied is False,
    ...
    "phase22_previous_status_archive_lock_review_complete": phase22_previous_status == "project_final_archive_lock_review_complete",
    ...
    "monitoring_not_started": lock_review.get("monitoring_started") is False,
    "run_dry_run_now_false": lock_review.get("run_dry_run_now") is False,
    "run_backtest_now_false": lock_review.get("run_backtest_now") is False,
    "execution_allowed_false": lock_review.get("execution_allowed") is False,
    "approved_for_execution_false": lock_review.get("approved_for_execution") is False,
    "approved_for_paper_shadow_false": lock_review.get("approved_for_paper_shadow") is False,
    "approved_for_live_false": lock_review.get("approved_for_live") is False,
    "paper_shadow_not_started": lock_review.get("paper_shadow_started") is False,
    "paper_shadow_start_not_approved": lock_review.get("approved_for_paper_shadow_start") is False,
    "exchange_order_submission_disabled": lock_review.get("exchange_order_submission") is False,
    "micro_live_not_approved": lock_review.get("approved_for_micro_live_execution") is False,
    "real_live_not_approved": lock_review.get("approved_for_real_live_trading") is False,
}

blockers = [key for key, value in completion_checks.items() if value is not True]
project_final_completion_closeout_passed = all(completion_checks.values())

if project_final_completion_closeout_passed:
    decision = "PHASE_22_PROJECT_FINAL_COMPLETION_CLOSEOUT_COMPLETE_REMAIN_ON_HOLD_NOT_APPROVED_FOR_EXECUTION"
    next_phase = "Phase 22.23 — Project Final Completion Review"
else:
    decision = "PHASE_22_PROJECT_FINAL_COMPLETION_CLOSEOUT_FAILED_REVIEW_REQUIRED"
    next_phase = "Phase 22.23 — Project Final Completion Closeout Fix"

closeout_file = PHASE22_DIR / "project_final_completion_closeout.json"

record = {
    "phase": "phase_22_22_project_final_completion_closeout_record",
    ...
    "phase22_status": "project_final_completion_closeout_complete",
    "selected_phase21_next_action": selected_phase21_next_action,
    "archive_lock_type": archive_lock_type,
    "os_file_locking_applied": False,
    "project_final_completion_closeout_passed": project_final_completion_closeout_passed,
    ...
    "final_project_state": {
        ...
        "phase22_status": "project_final_completion_closeout_complete",
        "selected_phase21_next_action": selected_phase21_next_action,
        "archive_status": "logically_locked_and_closed",
...
report = {
    "phase": "phase_22_22_project_final_completion_closeout",
    "generated_at_unix": int(time.time()),
    "scope": "project_final_completion_closeout_only_no_monitoring_no_dry_run_no_backtest_no_execution",
    "git_head": head,
    "git_branch": branch,
    "git_remote_origin": remote,
    "safety_flags": flags,
...
print(f"project_final_completion_closeout_passed={project_final_completion_closeout_passed}")
...
print("phase22_status=project_final_completion_closeout_complete")
print(f"selected_phase21_next_action={selected_phase21_next_action}")
print(f"archive_lock_type={archive_lock_type}")
