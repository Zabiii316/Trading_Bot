...
OUT = Path("data/processed/phase22_project_final_end_state_record.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_end_state_record_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
END_STATE_FILE = ARCHIVE_DIR / "project_final_end_state_record.json"

CLOSED_HOLD_REVIEW_FILES = [
    Path("data/processed/phase22_project_final_closed_hold_state_seal_review.json"),
    Path("runtime/phase22_project_final_closed_hold_state_seal_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_closed_hold_state_seal_review.json"),
]
...
reviews = [load(p) for p in CLOSED_HOLD_REVIEW_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

closed_hold_state_review_passed = (
    any_true(reviews, "project_final_closed_hold_state_seal_review_passed")
    or any("PHASE_22_PROJECT_FINAL_CLOSED_HOLD_STATE_SEAL_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in reviews)
    or any(x.get("phase22_status") == "project_final_closed_hold_state_seal_review_complete" for x in reviews)
)

project_status = first_value(reviews, "project_status", "remain_on_hold_not_approved_for_execution")
phase20_status = first_value(reviews, "phase20_status", "closed_remain_on_hold")
phase21_status = first_value(reviews, "phase21_status", "closed_final_remain_on_hold")
selected_action = first_value(reviews, "selected_phase21_next_action", "remain_on_hold")
archive_lock_type = first_value(reviews, "archive_lock_type", "logical_record_only")
os_file_locking = first_value(reviews, "os_file_locking_applied", False)

evidence_count = first_value(reviews, "evidence_file_count", manifest.get("evidence_file_count", 1))
runtime_count = first_value(reviews, "runtime_file_count", manifest.get("runtime_file_count", 1))
doc_count = first_value(reviews, "documentation_file_count", manifest.get("documentation_file_count", 1))
...
checks = {
    ...
    "closed_hold_state_seal_review_present": any(p.exists() for p in CLOSED_HOLD_REVIEW_FILES),
    "project_final_closed_hold_state_seal_review_passed": closed_hold_state_review_passed,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_END_STATE_RECORD_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_30_project_final_end_state_record",
    ...
    "project_final_end_state_record_created": passed,
    "final_end_state": {
        "bot_execution_state": "fully_on_hold",
        "monitoring_started": False,
        "run_dry_run_now": False,
        "run_backtest_now": False,
        "paper_shadow_started": False,
        "paper_shadow_state": "not_started",
    ...
    "approved_for_real_live_trading": False,
    "decision": decision,
    "next_phase": "Project remains closed on hold. No execution phase is approved.",
}

write(OUT, record)
write(RUNTIME_OUT, record)
write(END_STATE_FILE, record)

print(f"Report written to: {OUT}")
print(f"Runtime end state written to: {RUNTIME_OUT}")
print(f"End state file written to: {END_STATE_FILE}")
print(f"safe_mode_active={safe_mode}")
print(f"project_final_end_state_record_created={passed}")
...
print("phase22_status=project_final_end_state_record_created")
