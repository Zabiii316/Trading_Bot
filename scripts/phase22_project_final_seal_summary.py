...
OUT = Path("data/processed/phase22_project_final_seal_summary.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_seal_summary_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
SUMMARY_FILE = ARCHIVE_DIR / "project_final_seal_summary.json"

COMPLETION_REVIEW_FILES = [
    Path("data/processed/phase22_project_final_completion_review.json"),
    Path("runtime/phase22_project_final_completion_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_completion_review.json"),
]
...
FINAL_SEAL = Path("data/processed/phase22_project_final_evidence_seal.json")
...
reviews = [load(p) for p in COMPLETION_REVIEW_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)
final_seal = load(FINAL_SEAL)

completion_review_passed = (
    any_true(reviews, "project_final_completion_review_passed")
    or any("PHASE_22_PROJECT_FINAL_COMPLETION_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in reviews)
    or any(x.get("phase22_status") == "project_final_completion_review_complete" for x in reviews)
)
...
checks = {
    ...
    "completion_review_present": any(p.exists() for p in COMPLETION_REVIEW_FILES),
    ...
    "final_evidence_seal_present": FINAL_SEAL.exists(),
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_SEAL_SUMMARY_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_24_project_final_seal_summary",
    ...
    "project_final_seal_summary_created": passed,
    "final_project_summary": {
        ...
        "evidence_status": "sealed_and_summarized",
        "latest_safe_next_action": "remain_on_hold"
    },
    ...
    "decision": decision,
    "next_phase": "Phase 22.25 — Project Final Seal Summary Review",
}

write(OUT, record)
write(RUNTIME_OUT, record)
write(SUMMARY_FILE, record)

print(f"Report written to: {OUT}")
print(f"Runtime summary written to: {RUNTIME_OUT}")
print(f"Summary file written to: {SUMMARY_FILE}")
print(f"safe_mode_active={safe_mode}")
print(f"project_final_seal_summary_created={passed}")
...
print("phase22_status=project_final_seal_summary_created")
