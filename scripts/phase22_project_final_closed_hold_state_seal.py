...
OUT = Path("data/processed/phase22_project_final_closed_hold_state_seal.json")
RUNTIME_OUT = Path("runtime/phase22_project_final_closed_hold_state_seal_state.json")
ARCHIVE_DIR = Path("data/processed/phase22_project_archive")
SEAL_FILE = ARCHIVE_DIR / "project_final_closed_hold_state_seal.json"

HOLD_CLOSURE_REVIEW_FILES = [
    Path("data/processed/phase22_project_final_hold_closure_review.json"),
    Path("runtime/phase22_project_final_hold_closure_review_state.json"),
    Path("data/processed/phase22_project_archive/project_final_hold_closure_review.json"),
]
...
reviews = [load(p) for p in HOLD_CLOSURE_REVIEW_FILES]
manifest = load(MANIFEST)
archive_index = load(INDEX)

hold_closure_review_passed = (
    any_true(reviews, "project_final_hold_closure_review_passed")
    or any("PHASE_22_PROJECT_FINAL_HOLD_CLOSURE_REVIEW_COMPLETE" in str(x.get("decision", "")) for x in reviews)
    or any(x.get("phase22_status") == "project_final_hold_closure_review_complete" for x in reviews)
)
...
checks = {
    ...
    "hold_closure_review_present": any(p.exists() for p in HOLD_CLOSURE_REVIEW_FILES),
    "project_final_hold_closure_review_passed": hold_closure_review_passed,
...
decision = (
    ...
    if passed else
    "PHASE_22_PROJECT_FINAL_CLOSED_HOLD_STATE_SEAL_FAILED_REVIEW_REQUIRED"
)

record = {
    "phase": "phase_22_28_project_final_closed_hold_state_seal",
    ...
    "project_final_closed_hold_state_seal_created": passed,
    "closed_hold_state": {
    ...
    "approved_for_real_live_trading": False,
    "decision": decision,
    "next_phase": "Phase 22.29 — Project Final Closed Hold State Seal Review",
}

write(OUT, record)
write(RUNTIME_OUT, record)
write(SEAL_FILE, record)

print(f"Report written to: {OUT}")
print(f"Runtime seal written to: {RUNTIME_OUT}")
print(f"Seal file written to: {SEAL_FILE}")
print(f"safe_mode_active={safe_mode}")
print(f"project_final_closed_hold_state_seal_created={passed}")
...
print("phase22_status=project_final_closed_hold_state_seal_created")
